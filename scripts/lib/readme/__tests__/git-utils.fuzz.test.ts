...
describe('git-utils fuzzing', () => {
  ...
  describe('parseGitUrl', () => {
    test.prop([safeStringGenerator(), safeStringGenerator()])('should parse HTTPS GitHub URLs', (user, repo) => {
      const url = `https://github.com/${user}/${repo}.git`;
      const result = parseGitUrl(url);
      expect(result).toBe(`${user}/${repo}`);
    });

    test.prop([safeStringGenerator(), safeStringGenerator()])('should parse SSH GitHub URLs', (user, repo) => {
      const url = `git@github.com:${user}/${repo}.git`;
      const result = parseGitUrl(url);
      expect(result).toBe(`${user}/${repo}`);
    });

    test.prop([safeStringGenerator(), safeStringGenerator()])(
    ...
    test.prop([fc.string()])('should throw on invalid URLs', (invalidUrl) => {
