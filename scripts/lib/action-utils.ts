...
export async function removeVerifyWorkflow(packageName: string, subAction: string) {
  ...
  if (Sys.exists(workflowPath)) {
    ...
    console.log(
      chalk.green(`Removed verify workflow: .github/workflows/verify-action-${packageName}-${subAction}.yaml`),
    );
  } else {
    console.log(
      chalk.blue(`Verify workflow not found: .github/workflows/verify-action-${packageName}-${subAction}.yaml`),
    );
  }
}
