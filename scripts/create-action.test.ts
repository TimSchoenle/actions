...
describe('create-action', () => {
    ...
    it('should create new action in existing package', async () => {
        ...
        vi.mocked(input).mockResolvedValueOnce('new-sub') // Sub-action
        ...
        expect(Sys.mkdir).toHaveBeenCalledWith(
            expect.stringContaining('existing-pkg'),
            { recursive: true }
        );
        ...
        expect(createFromTemplate).not.toHaveBeenCalledWith(
            'action/CHANGELOG.md',
            expect.any(String),
            expect.any(Object)
        );
    ...
    it('should create new action and package files if new package', async () => {
        ...
        vi.mocked(input).mockResolvedValueOnce('sub')
