 ...
 * problems come from. Making them exclusive keeps a workflow from setting both and silently getting
 * whichever the action happened to prefer.
 */
 ...
 *
 * An empty result is refused rather than posted: a comment holding nothing but the hidden marker
 * renders as a blank comment, and every later run would then find and rewrite that blank — a
