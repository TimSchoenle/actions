...
function mockOctokit(): OctokitMock {
  const octokit: OctokitMock = {
    rest: {
      git: { createRef: vi.fn(), getRef: vi.fn(), updateRef: vi.fn() },
      repos: { get: vi.fn() },
    },
  };
...
describe('createBranchApi', () => {
  ...
  // The exact-match endpoint: the legacy plural one prefix-matches, so 'feat' would come back as
  // existing whenever 'feature/x' does.
