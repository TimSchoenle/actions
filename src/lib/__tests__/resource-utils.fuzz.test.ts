...
describe('resource-utils fuzzing', () => {
  ...
  const safePackageName = safeStringGenerator('abcdefghijklmnopqrstuvwxyz0123456789-');
  const safeSubName = safeStringGenerator('abcdefghijklmnopqrstuvwxyz0123456789-');
  ...
  describe('generateComponentName', () => {
    ...
    it('should handle hyphens in package and sub names', () => {
      const component1 = generateComponentName('action', 'my-package', 'my-sub');
      expect(component1).toBe('actions-my-package-my-sub');

      const component2 = generateComponentName('workflow', 'test-pkg', 'sub-action');
      expect(component2).toBe('workflows-test-pkg-sub-action-meta');
    });
